...
| 2026-08-02 | `IAzureCliCommandGateway` (allowlist tipada de operaciones de solo lectura/validación) como único mecanismo de ejecución de Azure CLI; plantillas Bicep de DEV validadas localmente (`bicep build`/`lint`, `deployment sub validate`/`what-if`), sin crear ningún recurso | Cumplir CLAUDE.md sección 5 (validar antes de ejecutar, sin comandos improvisados) manteniendo trazabilidad y auditoría de cada invocación | [ADR-002](adr/ADR-002-azure-cli-bicep.md) |
