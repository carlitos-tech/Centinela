...
En construcción por fases. **Fase 02 — Walking skeleton local fusionada** ([Pull Request #4](https://github.com/carlitos-tech/Centinela/pull/4), fusionado). Issue de la fase: [#3](https://github.com/carlitos-tech/Centinela/issues/3) (cerrado). **Fase 03 — Azure CLI Command Gateway y Bicep IaC fusionada** ([Pull Request #6](https://github.com/carlitos-tech/Centinela/pull/6), fusionado). Issue de la fase: [#5](https://github.com/carlitos-tech/Centinela/issues/5) (cerrado). **Fase 04 — Bootstrap Azure DEV: en preparación, aún no desplegada.** Issue de la fase: [#7](https://github.com/carlitos-tech/Centinela/issues/7).
...
La Fase 03 agregó `IAzureCliCommandGateway` (única vía permitida para ejecutar Azure CLI desde el código de Centinela, con allowlist tipada de operaciones de solo lectura/validación, auditoría y redacción automática de salida) y plantillas Bicep de DEV validadas localmente (`bicep build`/`lint`, `deployment sub validate`/`what-if`). **No se creó ningún recurso de Azure, no se registró ningún proveedor y no se ejecutó ningún `deployment ... create`.** Detalle completo en el [reporte de evidencia de la Fase 03](docs/evidence/phase-03-azure-cli-gateway-iac-report.md).

La Fase 04 (en preparación) endurece las plantillas Bicep heredadas de la Fase 03 (nombres globalmente únicos vía `uniqueString()`, `allowBlobPublicAccess=false` en Storage, regla de firewall `AllowAzureServices` de Azure SQL convertida en opt-in y deshabilitada por defecto), actualiza la estimación de costos, y agrega un script de despliegue protegido (`infra/scripts/deploy-dev.ps1`, con guardas probadas y **nunca ejecutado**) más una previsualización `what-if` de solo lectura (9 recursos `Create`, 0 `Delete`, 0 `Modify`). **Sigue sin crearse ningún recurso de Azure real.** Detalle completo en el [reporte de evidencia de preparación de la Fase 04](docs/evidence/phase-04-bootstrap-azure-dev-predeployment-report.md).
